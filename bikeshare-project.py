...
def get_filters():
    ...
    while True:
            if city == '1' or city == 'chicago':
                print("\nChicago, Let's go deeper\n")
                return 'chicago'
            if city == '2' or city == 'new york':
                print("\nNew York City, let's go deeper\n")
                return 'new york city'
            elif city == '3' or city == 'washington':
                print("\nWashington, let's go deeper\n")
                return 'washington'
            # error handled by 'else' and provided another option to input a response
            else:
...
def get_time():
    '''
    the code below asks the user to choose between month, and day of the week or no time filters.
    '''
    period = input('\nYou want to filter the data by month, day of the week, or you do not want to filter at all? Type "no" for no time filter.\n')
    period = period.lower()
...
def month_info(m):      # user response for month
    if m == 'month':
        month = input('\nChoose month! January, February, March, April, May, or June? type the full month name.\n')
        while month.strip().lower() not in ['january', 'february', 'march', 'april', 'may', 'june']:
            month = input('\nPlease choose between January, February, March, April, May, or June? type the full month name.\n')
        return month.strip().lower()
    else:
        return 'none'

def month_day_info(df, day_m):     # Asking for a month
    month_day = []
    if day_m == "day_of_month":
        ...
        while (True):
            ...
            try:
                ...
                print("use the correct value type!! (Numeric)")
    else:
        return 'none'

def day_info(d):       # Asks for a day and returns the day
    if d == 'day_of_week':
...
def time_filters(df, time, month, week_day, md):
    ...
    Local Variables:
    df         - a specified city dataframe
    time       - the specified time period indicated (either "month", "day_of_month", or "day_of_week")
    month      - shows the month chosen to filter the data
    week_day   - shows the week chosen to filter the data
...
def main():
    ...
    disp_raw_data(df)
        ...
        # the conclusions & functions
    stats_funcs_list = [month_freq,
     day_freq, hour_freq,
     trip_duration_stats, common_trip,
     stations_stats, user_stats, birth_years, gender_data]

    for x in stats_funcs_list:	# displays processing time for each function block
        process(x, df)

    # Restarting (option) for further analysis
